...
from loss.arcface_loss import ArcMarginProduct
...
def get_hidden_size_for_arcface(model):
    ...
    raise ValueError("Cannot infer hidden_size for ArcFace head from model config.")


def attach_arcface_head(model, training_args):
    if not training_args.use_arc_loss:
        return model

    if training_args.arc_num_labels <= 0:
        raise ValueError(
            "--use_arc_loss True nhưng --arc_num_labels <= 0. "
            "Hãy set đúng số lượng label_id."
        )

    hidden_size = get_hidden_size_for_arcface(model)

    model.arcface_head = ArcMarginProduct(
        in_features=hidden_size,
        out_features=training_args.arc_num_labels,
        s=training_args.arc_s,
        m=training_args.arc_m,
    )

    model.arcface_head.to(device=training_args.device, dtype=torch.float32)

    for name, param in model.named_parameters():
        if "arcface_head" in name:
            param.requires_grad = True
            rank0_print("[ArcFace trainable]", name, param.shape)

    rank0_print(
        f"[ArcFace] enabled | hidden_size={hidden_size} | "
        f"num_labels={training_args.arc_num_labels} | "
        f"s={training_args.arc_s} | m={training_args.arc_m} | "
        f"weight={training_args.arc_loss_weight} | pooling={training_args.arc_pooling}"
    )
