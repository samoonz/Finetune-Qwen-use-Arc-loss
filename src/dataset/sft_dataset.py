...
class SupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "label_id" in self.list_data_dict[i]:
            data_dict["arc_labels"] = torch.tensor(
                int(self.list_data_dict[i]["label_id"]),
                dtype=torch.long
            )
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, examples):
        ...
        for example in examples:
            ...
            if "arc_labels" in keys:
                batch_arc_labels.append(example["arc_labels"])
        ...
        if len(batch_arc_labels) > 0:
            data_dict["arc_labels"] = torch.stack(batch_arc_labels, dim=0)
