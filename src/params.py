...
class TrainingArguments(HFTrainingArguments):
    ...
    # ArcFace auxiliary classification loss
    use_arc_loss: bool = field(
        default=False,
        metadata={"help": "Enable auxiliary ArcFace loss during SFT."}
    )

    arc_num_labels: int = field(
        default=0,
        metadata={"help": "Number of unique label_id classes for ArcFace."}
    )

    arc_s: float = field(
        default=30.0,
        metadata={"help": "ArcFace scale factor."}
    )

    arc_m: float = field(
        default=0.30,
        metadata={"help": "ArcFace angular margin."}
    )

    arc_loss_weight: float = field(
        default=0.05,
        metadata={"help": "Weight of ArcFace loss added to SFT loss."}
    )

    arc_pair_batch: bool = field(
        default=False,
        metadata={"help": "Use pair_id-grouped batch sampler so RGB and segment pairs are in the same batch."}
    )

    arc_pairs_per_batch: int = field(
        default=4,
        metadata={"help": "Number of pair_id groups per batch."}
    )

    arc_samples_per_pair: int = field(
        default=2,
        metadata={"help": "Number of samples per pair_id. Usually 2: RGB + segment."}
    )

    arc_drop_incomplete_pairs: bool = field(
        default=True,
        metadata={"help": "Drop pair_id groups with fewer than arc_samples_per_pair samples."}
    )    
    arc_pooling: str = field(
        default="prompt",
        metadata={"help": "Pooling mode for ArcFace features: prompt, answer, all, last."}
    )
    arc_group_batch: bool = field(
        default=False,
        metadata={"help": "Use label-grouped batch sampler for ArcFace training."}
    )

    arc_labels_per_batch: int = field(
        default=4,
        metadata={"help": "Number of different labels per batch for grouped ArcFace batch."}
    )

    arc_samples_per_label: int = field(
        default=2,
        metadata={"help": "Number of samples per label in each grouped ArcFace batch."}
    )
